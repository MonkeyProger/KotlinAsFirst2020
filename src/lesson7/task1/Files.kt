...
fun victoriousMove(inputName: String, priority: Char): Pair<Int, Int>? {
    ...
    val plField = mutableMapOf<Int, MutableList<Char>>()
    File(inputName).bufferedReader().use {
        var y = 0
        it.forEachLine { line ->
            for (x in 0..14)
                plField.getOrPut(y) { mutableListOf() }.add(line[x])
            y++
        }
    }

    //Функция проверки диагоналей
    fun diagonalCheck(turn: Char, offsetX: Int, offsetY: Int): Pair<Int, Int>? {
        var toright = 0
        var toleft = 0
        var positionR = -1 to -1
        var positionL = -1 to -1
        for (i in 0..4) {
            when (plField[i + offsetY]?.get(i + offsetX)) {
                turn -> toright++
                '-' -> positionR = i + offsetX to i + offsetY
                else -> toright--
            }
            when (plField[i + offsetY]?.get(4 - i + offsetX)) {
                turn -> toleft++
    ...
    for (y in 0..10)
        for (x in 0..10) {
            ...
            val strip = stripCheck(priority, x, y)
